...
export class CDUGPSMonitor {
  static ShowPage(mcdu: LegacyFmsPageInterface, merit1?, merit2?, sat1?, sat2?) {
    ...
    mcdu.setTemplate([
      ...
      [`${currPos}[color]green`],
      ['TTRK', 'GS', 'UTC\xa0\xa0'],
      [`${Math.round(TTRK)}[color]green`, `${Math.round(GROUNDSPEED)}[color]green`, `${UTC}\xa0[color]green`],
      ['MERIT', 'MODE/SAT', 'GPS ALT\xa0\xa0'],
      [`${merit1}FT[color]green`, `NAV/${sat2}\xa0[color]green`, `${Math.round(ALTITUDE)}[color]green`],
      ['GPS2 POSITION'],
      [`${currPos}[color]green`],
      ['TTRK', 'GS', 'UTC\xa0\xa0'],
      [`${Math.round(TTRK)}[color]green`, `${Math.round(GROUNDSPEED)}[color]green`, `${UTC}\xa0[color]green`],
      ['MERIT', 'MODE/SAT', 'GPS ALT\xa0\xa0'],
      [`${merit2}FT[color]green`, `NAV/${sat2}\xa0[color]green`, `${Math.round(ALTITUDE)}[color]green`],
    ]);
